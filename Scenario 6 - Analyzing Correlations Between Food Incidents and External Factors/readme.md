Analyzing Correlation Between Food Safety Incidents and External Factors

# Narrative
Collected data from food incidents usually do not have a story to tell on their own. Appropriate processing is required to uncover meaningful patterns and relationships. By analyzing correlations between food safety incidents and external factors or even in between incidents, we can gain valuable insights. These insights help identify root causes, predict future risks, and support data-driven decision-making for improving food safety and public health outcomes.

# Scenario Overview

In the following scenario, we will utilize **Correlation Detective** to analyze correlation between food safety incidents on a dataset curating a set of incidents regarding various
food product categories, such as meat, dair, eggs and vegetables. 
...
<div style="text-align: center;">
    <img src="https://raw.githubusercontent.com/stelar-eu/walkthroughs/refs/heads/main/Scenario%206%20-%20Analyzing%20Correlations%20Between%20Food%20Incidents%20and%20External%20Factors/images/correlation-detective.png" alt="Data Pipeline Overview" style="width:700px;">
    <p><strong>Figure 1</strong>: Overview of the extraction of correlation in-between food safety incidents on a per month basis.</p>
</div>
<br>
...
<div style="text-align: center;">
    <a name="button" class="btn btn-primary btn-blue btn-pill py-1 px-3" href="https://github.com/stelar-eu/walkthroughs/blob/main/Scenario%206%20-%20Analyzing%20Correlations%20Between%20Food%20Incidents%20and%20External%20Factors/scenario6.ipynb" target="_blank" download>
...
process = client.processes.create(
    name="process-jsmith-correlation-detective",
    title="Correlation Detective Process for John Smith",
...
t.o(
    output_file =  {
        ...
        "url": "s3://klms-bucket/experiments/food_inc_correlations.json",
        # Specify the destination dataset for this resource
        "dataset": "d0"
        # Specify the resource metadata to be published after task completion
        "resource":{
            "name": "Food Safety Incidents Correlation Results for Monthly Dataset",
            "format": "JSON",
            "description": "This resource contains the results of the correlation analysis between food safety incidents and external factors.",
        }
    }
)
t.p(
...
```python
process = c.processes["process-jsmith-correlation-detective"]
...
2.**Open the Task** which is last executed in the process. The task name depends on the goal of the task, but it should be something like `Food incidents correlations for jsmith`.
