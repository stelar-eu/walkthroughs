# Narrative
Food safety experts constantly need to handle lengthy textual data, such as food recall announcements, in big batches, from various sources and in various languages. To exploit this information appropriately, they need to be able to efficiently and effectively analyze those data, identify the affected food and map them to the internal ontologies they use for their processes, regardless of the language in which the initial announcement was published in.

# Scenario Overview

In the following scenario, we utilize **GenericNER** to analyze food recall announcements, identify the affected foods mentioned inside them and map link them to a provided ontology that the user prefers.
...
- **Filter By Tags** - Check for **`Annotation`** or **`Generic-NER`** or `Food Recall`
<div style="text-align: center;">
...
1. <a href="/stelar/console/v1/catalog/generic-ner-test-sample" target="_blank">Generic NER Test Sample</a> - A dataset for testing Named Entity Recognition tools.
...
<div style="text-align: center;">
    <a name="button" class="btn btn-primary btn-blue btn-pill py-1 px-3" href="https://github.com/stelar-eu/walkthroughs/blob/main/Scenario%207%20-%20Extracting%20Food%20Entities%20out%20of%20Food%20Recall%20Announcements/scenario7.ipynb" target="_blank" download>
...
```python
dataset_n = c.datasets["generic-ner-test-sample"]
...
process = client.processes.create(
    name="process-jsmith-named-entity-recognition",
    title="Named Entity Recognition Process for John Smith",
...
t.o(
    output_file =  {
        ...
        "url": "s3://klms-bucket/experiments/annotations_<user>.csv",
        # Specify the destination dataset for this resource
        "dataset": "d0"
        # Specify the resource metadata to be published after task completion
        "resource":{
            "name": "Food Recall Announcements Annotations",
            "format": "CSV",
            "description": "This resource contains the annotations extracted from food recall announcements.",
        }
    }
)
t.p(
...
```python
process = c.processes["process-jsmith-named-entity-recognition"]
...
2.**Open the Task** which is last executed in the process. The task name depends on the goal of the task, but it should be something like `Annotated NER Test Dataset for jsmith`.
